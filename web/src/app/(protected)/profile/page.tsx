...
export default async function ProfilePage() {
  ...
  const joinedDate = profile
    ? new Date(profile.created_at).toLocaleDateString('uk-UA', { year: 'numeric', month: 'long' })
    : ''
  ...
  return (
    <div className="max-w-3xl mx-auto pb-10">
      {/* Cover + avatar hero */}
      <div className="relative mx-6 mt-6 rounded-2xl overflow-hidden shadow-sm">
        {/* Cover banner */}
        ...
        <div className="bg-white px-6 pb-5">
          <div className="flex items-end justify-between gap-4 -mt-10 flex-wrap">
            ...
            <Link
              href="/profile/edit"
              className="inline-flex items-center gap-1.5 text-sm px-4 py-2 rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50 transition font-medium mb-1"
            >
          ...
          <div className="mt-3">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5 text-sm text-gray-500">
              ...
              {joinedDate && (
                <span className="text-xs text-gray-400">🗓 На платформі з {joinedDate}</span>
              )}
            </div>
            ...
            {!profile?.bio && !profile?.city && (
              <p className="text-sm text-gray-400 mt-3 italic">
                Профіль ще не заповнено.{' '}
                <Link href="/profile/edit" className="text-blue-500 hover:underline not-italic">
                  Заповнити зараз →
                </Link>
              </p>
            )}
      ...
      <div className="mx-6 mt-3 grid md:grid-cols-2 gap-3">
        ...
        {myCommunities && myCommunities.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h2 className="text-sm font-semibold text-gray-800 mb-3 flex items-center gap-2">
              <span className="text-base">👥</span> Мої спільноти
            </h2>
            ...
            {(communityCount ?? 0) > 4 && (
              <Link href="/communities" className="block text-xs text-blue-500 hover:underline mt-2 text-center">
                Переглянути всі →
              </Link>
            )}
          </div>
